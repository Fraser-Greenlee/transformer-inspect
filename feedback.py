'''
    Convert GPT-2 into a recurrent model to improve performance on reasoning tasks.

    This uses less memory & more compute.
'''
import torch
...
def use_feedback(self, feedback_window_size=1):
    '''
        Replace `self.forward` with `alt_forward`.
        Keeps `self` as a closure.
    '''
    ...
    def alt_forward(
        ...
        if self.config.add_cross_attention or output_attentions or use_cache or self.model_parallel:
            raise NotImplementedError(
                f'Not implimented with feedback window. self.config.add_cross_attention={self.config.add_cross_attention}'
        ...
        for feedback_i in range(0, hidden_states.size(1), feedback_window_size):
            ...
            for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
                ...
                if getattr(self.config, "gradient_checkpointing", False) and self.training:
                    raise NotImplementedError('Not implimented with feedback window.')
